...
// frameworks/keyed/react-hooks reference). Dispatch is wrapped in `flushSync`
// so React commits the update SYNCHRONOUSLY inside the discrete click — the
// harness times only the synchronous click handler, so without this the timer
// would capture ~0ms of scheduling instead of the actual render.
...
const Main = () => {
	...
	// Commit synchronously inside the click so the sync-timed harness captures
	// the real render. Stable identity keeps the memo'd children.
