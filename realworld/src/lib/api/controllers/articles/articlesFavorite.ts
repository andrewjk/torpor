import { notFound, ok, serverError, unauthorized } from "@tera/build/response";
import articleFavoritePrisma from "../../db/article/articleFavoritePrisma";
...
export default async function articlesFavorite(
	...
	try {
		...
		if (!currentUser) return serverError(); // The user should not have disappeared after having favorited an article
