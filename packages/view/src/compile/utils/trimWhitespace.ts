...
export default function trimWhitespace(node: ContainerNode): void {
	trimChildren(node.children, false);
}

function trimChildren(children: TemplateNode[], preserveAll: boolean): void {
	...
	for (const child of children) {
		...
		if (grandChildren) {
			const childPreserve = preserveAll || PRESERVE_TAGS.has(getTagName(child) ?? "");
			trimChildren(grandChildren, childPreserve);
		}
	}
