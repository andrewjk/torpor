import {
	ANCHOR_COMMENT,
	HYDRATION_END_COMMENT,
	HYDRATION_START_COMMENT,
} from "../../types/comments";
import type ControlNode from "../../types/nodes/ControlNode";
import Builder from "../../utils/Builder";
import isControlNode from "../../utils/isControlNode";
import type BuildServerStatus from "./BuildServerStatus";
import buildServerNode from "./buildServerNode";

export default function buildServerLoadingNode(
	node: ControlNode,
	status: BuildServerStatus,
	b: Builder,
): void {
	// Surround the entire control statement with bracketed comments, so that we
	// can skip to the end to set the anchor node when hydrating
	status.output += HYDRATION_START_COMMENT;

	if (status.output) {
		b.append(`t_body += \`${status.output}\`;`);
		status.output = "";
	}

	// On the server, render the fallback branch (the loading state) since the
	// promise hasn't resolved yet. The client will hydrate and swap to content
	// when the $await getter resolves.
	for (let branch of node.children) {
		if (isControlNode(branch) && branch.operation === "@fallback") {
			buildServerLoadingBranch(branch, status, b);
		}
	}

	// End the control statement
	status.output += HYDRATION_END_COMMENT;

	// Add the anchor node
	status.output += ANCHOR_COMMENT;
}

function buildServerLoadingBranch(node: ControlNode, status: BuildServerStatus, b: Builder) {
	for (let child of node.children) {
		buildServerNode(child, status, b);
	}

	if (status.output) {
		b.append(`t_body += \`${status.output}\`;`);
		status.output = "";
	}
}
