...
export default function IfElseIf(
	...
	t_run_control(t_if_region_1, t_if_anchor_1, (t_before) => {
		if ($props.counter > 10) {
			...
			const t_fragment_1 = t_fragment_el($parent.ownerDocument!, t_fragment_els, 1, `<p> It's over ten! </p>`);
			...
			const t_new_region = t_region();
			const t_old_region = t_push_region(t_new_region, true);
			const t_fragment_2 = t_fragment_el($parent.ownerDocument!, t_fragment_els, 2, `<p> It's over five! </p>`);
			...
			const t_new_region = t_region();
			const t_old_region = t_push_region(t_new_region, true);
			const t_fragment_3 = t_fragment_el($parent.ownerDocument!, t_fragment_els, 3, `<p> It's not there yet </p>`);
