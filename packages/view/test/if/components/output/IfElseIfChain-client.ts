...
export default function IfElseIf(
	...
	t_run_control(t_if_region_1, t_if_anchor_1, (t_before) => {
		if ($props.count < 0) {
			...
			const t_fragment_1 = t_fragment($parent.ownerDocument!, t_fragments, 1, `<p>Negative</p>`);
			...
			const t_new_region = t_region();
			const t_old_region = t_push_region(t_new_region, true);
			const t_fragment_2 = t_fragment($parent.ownerDocument!, t_fragments, 2, `<p>Zero</p>`);
			...
			const t_new_region = t_region();
			const t_old_region = t_push_region(t_new_region, true);
			const t_fragment_3 = t_fragment($parent.ownerDocument!, t_fragments, 3, `<p>Small positive</p>`);
			...
			const t_new_region = t_region();
			const t_old_region = t_push_region(t_new_region, true);
			const t_fragment_4 = t_fragment($parent.ownerDocument!, t_fragments, 4, `<p>Large positive</p>`);
