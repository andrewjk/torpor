...
export default function IfContainingFor(
	...
	t_run_control(t_if_region_1, t_if_anchor_1, (t_before) => {
		if ($props.show) {
			...
			const t_ul_1 = t_root_el(t_fragment_1) as HTMLElement;
			...
			t_run_list(
				...
				(t_item_1, t_before_1) => {
					...
					const t_li_1 = t_root_el(t_fragment_2) as HTMLElement;
			...
			const t_new_region = t_region();
			const t_old_region = t_push_region(t_new_region, true);
			const t_fragment_3 = t_fragment_el($parent.ownerDocument!, t_fragment_els, 3, `<p>Nothing to show</p>`);
