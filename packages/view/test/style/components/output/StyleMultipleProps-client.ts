...
export default function StyleMultiple(
	...
	const t_fragment_0 = t_fragment_el($parent.ownerDocument!, t_fragment_els, 0, `<div id="multi"> Multi style </div>`);
	const t_div_1 = t_root_el(t_fragment_0) as HTMLDivElement;
	$run(() => {
		t_div_1.style.cssText += t_style({ color: $props.color, fontSize: $props.fontSize + "px", background: $props.background });
	});
