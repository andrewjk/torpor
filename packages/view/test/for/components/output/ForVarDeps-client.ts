	...
	t_run_list(
		...
		() => {
			...
			for (let { id, a, b } of $props.items) {
				t_new_items_1.push({ data: { id, a, b }, key:
				id });
			}
		...
		(t_item_1, t_before_1) => {
			...
			$run(() => {
				t_text_1.textContent = t_fmt(t_item_1.data.a);
		...
		(t_old_item, t_new_item) => {
			...
			if (t_old_item.data.id !== t_new_item.data.id) {
				t_old_item.data.id = t_new_item.data.id;
				t_changed_mask |= 1;
			}
			if (t_old_item.data.a !== t_new_item.data.a) {
				t_old_item.data.a = t_new_item.data.a;
				t_changed_mask |= 2;
			}
			if (t_old_item.data.b !== t_new_item.data.b) {
				t_old_item.data.b = t_new_item.data.b;
				t_changed_mask |= 4;
			}
