...
export default function ForSwitchReactive(
	...
	t_run_list(
		...
		(t_item_1, t_before_1) => {
			...
			let t_switch_anchor_1 = t_anchor(t_root_1) as HTMLElement;
			...
			t_run_control(t_switch_region_1, t_switch_anchor_1, (t_before) => {
				switch (t_item_1.data.item.status) {
					case "on": {
						...
						const t_fragment_2 = t_fragment_el($parent.ownerDocument!, t_fragment_els, 2, `<li class="on">#</li>`);
					...
					case "off": {
						...
						const t_new_region = t_region();
						const t_old_region = t_push_region(t_new_region, true);
						const t_fragment_3 = t_fragment_el($parent.ownerDocument!, t_fragment_els, 3, `<li class="off">#</li>`);
					...
					default: {
						...
						const t_new_region = t_region();
						const t_old_region = t_push_region(t_new_region, true);
						const t_fragment_4 = t_fragment_el($parent.ownerDocument!, t_fragment_els, 4, `<li class="unknown">#</li>`);
			...
			t_add_fragment(t_fragment_1, t_for_parent_1, t_before_1, t_switch_anchor_1, t_root_1);
