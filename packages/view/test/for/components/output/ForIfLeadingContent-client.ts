...
export default function ForIfLeadingContent(
	$parent: ParentNode,
	$anchor: Node | null,
	...
	t_run_list(
		...
		(t_item_1, t_before_1) => {
			...
			let t_if_anchor_1 = t_anchor(t_next(t_next(t_root_1, true))) as HTMLElement;
			...
			t_run_control(t_if_region_1, t_if_anchor_1, (t_before) => {
				if (t_item_1.data.t.done) {
					...
					const t_fragment_2 = t_fragment_el($parent.ownerDocument!, t_fragment_els, 2, `<b class="mark">#</b>`);
