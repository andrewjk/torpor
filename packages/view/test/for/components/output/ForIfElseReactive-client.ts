...
export default function ForIfElseReactive(
	$parent: ParentNode,
	$anchor: Node | null,
	...
	t_run_list(
		...
		(t_item_1, t_before_1) => {
			...
			t_run_control(t_if_region_1, t_if_anchor_1, (t_before) => {
				if (t_item_1.data.t.done) {
					...
					const t_fragment_2 = t_fragment_el($parent.ownerDocument!, t_fragment_els, 2, `<li class="done">#</li>`);
					...
					const t_new_region = t_region();
					const t_old_region = t_push_region(t_new_region, true);
					const t_fragment_3 = t_fragment_el($parent.ownerDocument!, t_fragment_els, 3, `<li class="todo">#</li>`);
		...
		(t_old_item, t_new_item) => {
			t_old_item.data.t = t_new_item.data.t;
		}
	);
