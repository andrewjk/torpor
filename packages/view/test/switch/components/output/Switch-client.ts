	...
	t_run_control(t_switch_region_1, t_switch_anchor_1, (t_before) => {
		switch ($props.value) {
			case 1: {
				...
				const t_fragment_1 = t_fragment_el($parent.ownerDocument!, t_fragment_els, 1, `<p> A small value. </p>`);
			...
			case 100: {
				...
				const t_new_region = t_region();
				const t_old_region = t_push_region(t_new_region, true);
				const t_fragment_2 = t_fragment_el($parent.ownerDocument!, t_fragment_els, 2, `<p> A large value. </p>`);
			...
			default: {
				...
				const t_new_region = t_region();
				const t_old_region = t_push_region(t_new_region, true);
				const t_fragment_3 = t_fragment_el($parent.ownerDocument!, t_fragment_els, 3, `<p> Another value. </p>`);
