	...
	t_slots_1["_"] = (
		...
		t_run_list(
			...
			(t_item_1, t_before_1) => {
				...
				let t_if_anchor_1 = t_anchor(t_next(t_root_3)) as HTMLElement;
				...
				t_run_control(t_if_region_1, t_if_anchor_1, (t_before) => {
					if (t_item_1.data.item.visible) {
						...
						const t_fragment_4 = t_fragment($parent.ownerDocument!, t_fragments, 4, ` <p>#</p> `);
						const t_root_4 = t_root(t_fragment_4, true);
						const t_text_1 = t_child(t_next(t_root_4));
				...
				t_add_fragment(t_fragment_3, t_fragment_2, t_before_1, t_text_3);
				t_next(t_text_3);
		...
		const t_text_4 = t_next(t_for_anchor_1, true);
		t_add_fragment(t_fragment_2, $sparent, $sanchor, t_text_4);
		t_next(t_text_4);
	}
	Card(t_fragment_0, t_comp_anchor_1, undefined, $context, t_slots_1);

	const t_text_5 = t_next(t_comp_anchor_1, true);
...
function Card(
	...
	const t_fragment_0 = t_fragment($parent.ownerDocument!, t_fragments, 0, ` <div class="card"> <h2>Card title</h2> <!> </div> `);
	const t_root_0 = t_root(t_fragment_0, true);
