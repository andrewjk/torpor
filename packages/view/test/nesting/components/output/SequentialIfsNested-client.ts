...
export default function SeqIfsNested(
	$parent: ParentNode,
	$anchor: Node | null,
	...
	t_run_control(t_if_region_1, t_if_anchor_1, (t_before) => {
		if ($props.a) {
			...
			const t_fragment_1 = t_fragment($parent.ownerDocument!, t_fragments, 1, `<p>A on</p> <!>`);
			...
			t_run_control(t_if_region_2, t_if_anchor_2, (t_before) => {
				if ($props.b) {
					...
					const t_new_region = t_region();
					const t_old_region = t_push_region(t_new_region, true);
					const t_fragment_2 = t_fragment($parent.ownerDocument!, t_fragments, 2, `<p>A+B on</p> <!>`);
					...
					t_run_control(t_if_region_3, t_if_anchor_3, (t_before) => {
						if ($props.c) {
							...
							const t_new_region = t_region();
							const t_old_region = t_push_region(t_new_region, true);
							const t_fragment_3 = t_fragment($parent.ownerDocument!, t_fragments, 3, `<p>A+B+C on</p>`);
	...
	let t_if_anchor_4 = t_anchor(t_next(t_next(t_if_anchor_1, true))) as HTMLElement;
	...
	t_run_control(t_if_region_4, t_if_anchor_4, (t_before) => {
		if ($props.b) {
			...
			const t_new_region = t_region();
			const t_old_region = t_push_region(t_new_region, true);
			const t_fragment_4 = t_fragment($parent.ownerDocument!, t_fragments, 4, `<p>B only section</p>`);
			const t_p_2 = t_root(t_fragment_4) as HTMLElement;
			t_add_fragment(t_fragment_4, t_fragment_0, t_before, t_p_2);
			t_next(t_p_2);
			t_pop_region(t_old_region);
			t_if_index_4 = 0;
		}
