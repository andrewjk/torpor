...
export default function SeqIfs(
	...
	t_run_control(t_if_region_1, t_if_anchor_1, (t_before) => {
		if ($props.a) {
			...
			const t_fragment_1 = t_fragment_el($parent.ownerDocument!, t_fragment_els, 1, `<p>A on</p>`);
	...
	t_run_control(t_if_region_2, t_if_anchor_2, (t_before) => {
		if ($props.b) {
			...
			const t_new_region = t_region();
			const t_old_region = t_push_region(t_new_region, true);
			const t_fragment_2 = t_fragment_el($parent.ownerDocument!, t_fragment_els, 2, `<p>B on</p>`);
	...
	t_run_control(t_if_region_3, t_if_anchor_3, (t_before) => {
		if ($props.c) {
			...
			const t_new_region = t_region();
			const t_old_region = t_push_region(t_new_region, true);
			const t_fragment_3 = t_fragment_el($parent.ownerDocument!, t_fragment_els, 3, `<p>C on</p>`);
