...
export default function DeepMixed(
						...
						if ($props.c) {
							...
							t_run_control(t_if_region_4, t_if_anchor_4, (t_before) => {
								if ($props.d) {
									...
									let t_if_anchor_5 = t_anchor(t_next(t_next(t_root_4, true))) as HTMLElement;
									...
									t_run_control(t_if_region_5, t_if_anchor_5, (t_before) => {
										if ($props.e) {
											...
											const t_new_region = t_region();
											const t_old_region = t_push_region(t_new_region, true);
											const t_fragment_5 = t_fragment_el($parent.ownerDocument!, t_fragment_els, 5, `<p>Level 5</p>`);
											const t_root_5 = t_root_el(t_fragment_5);
											const t_p_1 = t_root_5 as HTMLElement;
											t_add_element(t_p_1, t_fragment_4, t_before);
											t_next(t_p_1);
									...
									t_add_fragment(t_fragment_4, t_fragment_3, t_before, t_if_anchor_5, t_root_4);
	...
	t_run_control(t_if_region_6, t_if_anchor_6, (t_before) => {
		if ($props.a && $props.e) {
			...
			const t_new_region = t_region();
			const t_old_region = t_push_region(t_new_region, true);
			const t_fragment_6 = t_fragment_el($parent.ownerDocument!, t_fragment_els, 6, `<p>A+E</p>`);
			const t_root_6 = t_root_el(t_fragment_6);
			const t_p_2 = t_root_6 as HTMLElement;
	...
	t_add_fragment(t_fragment_0, $parent, $anchor, t_if_anchor_6, t_root_0);
	t_next(t_if_anchor_6);
