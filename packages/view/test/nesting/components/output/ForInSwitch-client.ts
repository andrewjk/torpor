...
export default function ForInSwitch(
	...
	t_run_control(t_switch_region_1, t_switch_anchor_1, (t_before) => {
		switch ($props.choice) {
			case "list": {
				...
				const t_for_parent_1 = t_next(t_root_1) as HTMLElement;
				...
				t_run_list(
					...
					(t_item_1, t_before_1) => {
						...
						t_add_fragment(t_fragment_2, t_for_parent_1, t_before_1, t_text_2);
						t_next(t_text_2);
			...
			case "count": {
				...
				$run(() => {
					t_text_4.textContent = `Count: ${t_fmt($props.items.length)}`;
				});
			...
			default: {
				...
				const t_new_region = t_region();
				const t_old_region = t_push_region(t_new_region, true);
				const t_fragment_4 = t_fragment($parent.ownerDocument!, t_fragments, 4, ` <p>Nothing</p> `);
	...
	const t_text_7 = t_next(t_switch_anchor_1, true);
