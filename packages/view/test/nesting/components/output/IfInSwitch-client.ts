	...
	t_run_control(t_switch_region_1, t_switch_anchor_1, (t_before) => {
		switch ($props.status) {
			case "active": {
				...
				t_run_control(t_if_region_1, t_if_anchor_1, (t_before) => {
					if ($props.show) {
						...
						const t_new_region = t_region();
						const t_old_region = t_push_region(t_new_region, true);
						const t_fragment_2 = t_fragment_el($parent.ownerDocument!, t_fragment_els, 2, `<p>Active and visible</p>`);
						...
						const t_new_region = t_region();
						const t_old_region = t_push_region(t_new_region, true);
						const t_fragment_3 = t_fragment_el($parent.ownerDocument!, t_fragment_els, 3, `<p>Active but hidden</p>`);
			...
			case "inactive": {
				...
				const t_new_region = t_region();
				const t_old_region = t_push_region(t_new_region, true);
				const t_fragment_4 = t_fragment_el($parent.ownerDocument!, t_fragment_els, 4, `<p>Inactive</p>`);
			...
			default: {
				...
				const t_new_region = t_region();
				const t_old_region = t_push_region(t_new_region, true);
				const t_fragment_5 = t_fragment_el($parent.ownerDocument!, t_fragment_els, 5, `<p>Unknown status</p>`);
				const t_p_4 = t_root_el(t_fragment_5) as HTMLElement;
