	...
	t_run_control(t_switch_region_1, t_switch_anchor_1, (t_before) => {
		switch ($props.status) {
			case "active": {
				...
				t_run_control(t_if_region_1, t_if_anchor_1, (t_before) => {
					if ($props.show) {
						...
						const t_new_region = t_region();
						const t_old_region = t_push_region(t_new_region, true);
						const t_fragment_2 = t_fragment($parent.ownerDocument!, t_fragments, 2, `<p>Active and visible</p>`);
						...
						const t_new_region = t_region();
						const t_old_region = t_push_region(t_new_region, true);
						const t_fragment_3 = t_fragment($parent.ownerDocument!, t_fragments, 3, `<p>Active but hidden</p>`);
			...
			case "inactive": {
				...
				const t_new_region = t_region();
				const t_old_region = t_push_region(t_new_region, true);
				const t_fragment_4 = t_fragment($parent.ownerDocument!, t_fragments, 4, `<p>Inactive</p>`);
			...
			default: {
				...
				const t_new_region = t_region();
				const t_old_region = t_push_region(t_new_region, true);
				const t_fragment_5 = t_fragment($parent.ownerDocument!, t_fragments, 5, `<p>Unknown status</p>`);
				const t_p_4 = t_root(t_fragment_5) as HTMLElement;
				t_add_fragment(t_fragment_5, t_fragment_0, t_before, t_p_4);
				t_next(t_p_4);
				t_pop_region(t_old_region);
				t_switch_index_1 = 2;
				break;
