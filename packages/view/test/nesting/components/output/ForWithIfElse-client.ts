		...
		(t_item_1, t_before_1) => {
			...
			t_run_control(t_if_region_1, t_if_anchor_1, (t_before) => {
				if (t_item_1.data.tab === $props.activeTab) {
					...
					const t_button_1 = t_root(t_fragment_2) as HTMLButtonElement;
					const t_text_1 = t_child(t_button_1);
					$run(() => {
						t_text_1.textContent = t_fmt(t_item_1.data.tab);
					});
					t_add_fragment(t_fragment_2, t_fragment_1, t_before, t_button_1);
					t_next(t_button_1);
					...
					const t_button_2 = t_root(t_fragment_3) as HTMLButtonElement;
					const t_text_2 = t_child(t_button_2);
					$run(() => {
						t_text_2.textContent = t_fmt(t_item_1.data.tab);
					});
					t_add_fragment(t_fragment_3, t_fragment_1, t_before, t_button_2);
					t_next(t_button_2);
					t_pop_region(t_old_region);
					t_if_index_1 = 1;
				}
