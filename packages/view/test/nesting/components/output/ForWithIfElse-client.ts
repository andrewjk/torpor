...
export default function ForWithIfElse(
		...
		(t_item_1, t_before_1) => {
			...
			t_run_control(t_if_region_1, t_if_anchor_1, (t_before) => {
				if (t_item_1.data.tab === $props.activeTab) {
					...
					const t_fragment_2 = t_fragment($parent.ownerDocument!, t_fragments, 2, ` <button class="active">#</button> `);
					...
					const t_new_region = t_region();
					const t_old_region = t_push_region(t_new_region, true);
					const t_fragment_3 = t_fragment($parent.ownerDocument!, t_fragments, 3, ` <button>#</button> `);
					const t_root_3 = t_root(t_fragment_3, true);
					const t_text_3 = t_child(t_next(t_root_3));
					const t_text_4 = t_next(t_next(t_root_3), true);
					$run(() => {
						t_text_3.textContent = t_fmt(t_item_1.data.tab);
					});
					t_add_fragment(t_fragment_3, t_fragment_1, t_before, t_text_4);
					t_next(t_text_4);
					t_pop_region(t_old_region);
					t_if_index_1 = 1;
				}
			...
			t_add_fragment(t_fragment_1, t_for_parent_1, t_before_1, t_text_5);
			t_next(t_text_5);
	...
	const t_text_6 = t_child(t_next(t_next(t_for_parent_1, true)));
	const t_text_7 = t_next(t_next(t_next(t_for_parent_1, true)), true);
	$run(() => {
		t_text_6.textContent = `Active: ${t_fmt($props.activeTab)}`;
	});
