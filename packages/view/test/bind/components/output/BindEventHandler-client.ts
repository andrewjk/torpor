...
export default function BindEventHandler(
	...
	const t_fragment_0 = t_fragment($parent.ownerDocument!, t_fragments, 0, `<input> <p>#</p> <!>`);
	const t_root_0 = t_root(t_fragment_0);
	const t_input_1 = t_root_0 as HTMLInputElement;
	const t_text_1 = t_child(t_next(t_next(t_input_1, true)));
	let t_if_anchor_1 = t_anchor(t_next(t_next(t_next(t_next(t_input_1, true)), true))) as HTMLElement;
	...
	t_run_control(t_if_region_1, t_if_anchor_1, (t_before) => {
		if ($state.typed) {
			...
			const t_fragment_1 = t_fragment_el($parent.ownerDocument!, t_fragment_els, 1, `<p>Handler ran</p>`);
	...
	t_event(t_input_1, "input", (e) => {
		((() => { $state.typed = true; }) as ((e: any) => any) | undefined)?.(e);
