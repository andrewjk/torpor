...
export default function SlotReactive(
	...
	const t_fragment_0 = t_fragment($parent.ownerDocument!, t_fragments, 0, ` <!> <button>Update</button> `);
	...
	t_slots_1["_"] = (
		...
		$run(() => {
			t_text_1.textContent = t_fmt($state.label);
		});
	...
	t_event(t_button_1, "click", () => $state.label = "Updated");
