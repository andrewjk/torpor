...
export default function SlotDefault(
	...
	t_slots_1["_"] = (
		...
		const t_fragment_2 = t_fragment($parent.ownerDocument!, t_fragments, 2, `<p>Default content</p>`);
		const t_p_1 = t_root(t_fragment_2) as HTMLElement;
		t_add_fragment(t_fragment_2, $sparent, $sanchor, t_p_1);
		t_next(t_p_1);
	}
	List(t_fragment_0, t_comp_anchor_1, undefined, $context, t_slots_1);
...
function List(
	...
	if ($slots && $slots["_"]) {
		...
		const t_fragment_1 = t_fragment($parent.ownerDocument!, t_fragments, 1, `<li>Empty list</li>`);
		const t_li_1 = t_root(t_fragment_1) as HTMLElement;
		t_add_fragment(t_fragment_1, t_ul_1, t_slot_anchor_1, t_li_1);
		t_next(t_li_1);
	}
