...
export default function IfNested(
	...
	t_run_control(t_if_region_1, t_if_anchor_1, (t_before) => {
		if ($props.counter > 5) {
			...
			let t_if_anchor_2 = t_anchor(t_root(t_fragment_1)) as HTMLElement;
			...
			t_run_control(t_if_region_2, t_if_anchor_2, (t_before) => {
				if ($props.counter > 10) {
					...
					const t_new_region = t_region();
					const t_old_region = t_push_region(t_new_region, true);
					const t_fragment_2 = t_fragment_el($parent.ownerDocument!, t_fragment_els, 2, `<p> It's both true! </p>`);
					...
					const t_new_region = t_region();
					const t_old_region = t_push_region(t_new_region, true);
					const t_fragment_3 = t_fragment_el($parent.ownerDocument!, t_fragment_els, 3, `<p> The second is not true! </p>`);
			...
			const t_new_region = t_region();
			const t_old_region = t_push_region(t_new_region, true);
			const t_fragment_4 = t_fragment_el($parent.ownerDocument!, t_fragment_els, 4, `<p> The first is not true! </p>`);
