...
export default function ServerAsyncMixed(
	...
	t_run_await(t_await_region_1, t_await_anchor_1, (t_before) => {
		...
		$run(() => {
			t_text_1.textContent = `${t_fmt($state.serverData)} / ${t_fmt($state.clientData)}`;
		});
		t_add_element(t_p_1, t_fragment_0, t_before);
		t_next(t_p_1);
	}, (t_before) => {
		const t_fragment_2 = t_fragment_el($parent.ownerDocument!, t_fragment_els, 2, `<p>Loading mixed...</p>`);
	...
	t_add_fragment(t_fragment_0, $parent, $anchor, t_await_anchor_1, t_root_0);
