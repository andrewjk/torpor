...
export default function TryCatchNested(
	...
	t_run_control(t_try_region_1, t_try_anchor_1, (t_before) => {
		...
		try {
			...
			let t_try_index_2 = -1;
			t_run_control(t_try_region_2, t_try_anchor_2, (t_before) => {
				const t_hydration_snapshot_2 = t_save_hydration();
				try {
					if (!t_run_branch(t_try_region_2, t_try_index_2, 0)) return;
					const t_new_region = t_region();
					const t_old_region = t_push_region(t_new_region, true);
					...
					t_push_region(t_try_region_2);
					t_restore_hydration(t_hydration_snapshot_2);
					t_try_index_2 = -1;
					if (!t_run_branch(t_try_region_2, t_try_index_2, 1)) return;
					const t_new_region = t_region();
					const t_old_region = t_push_region(t_new_region, true);
