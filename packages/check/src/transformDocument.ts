...
import pathReplace from "./pathReplace";
...
export default function transformDocument(
		...
		if (!parsed.ok) {
			return {
				...
				errors: parsed.errors.map((e) => {
					let lineTextStart = e.startIndex;
					while (lineTextStart >= 0) {
						if (source[lineTextStart] === "\n") {
							lineTextStart++;
							break;
						}
						lineTextStart--;
					}
					let lineTextEnd: number | undefined = source.indexOf("\n", e.startIndex);
					if (lineTextEnd === -1) lineTextEnd = undefined;
					return {
						path: filename,
						message: e.message,
						range: {
						...
						lineText: source.substring(lineTextStart, lineTextEnd),
...
function importComponentFiles(
	...
	for (let i = 0; i < first.script.length; i++) {
		if (first.script[i] === "/") {
			...
			if (vts.configPath && vts.config.options.paths) {
				// HACK: Get the longest match etc
				const oldTypeFile = typeFile;
				for (let [srcGlob, paths] of Object.entries(vts.config.options.paths)) {
