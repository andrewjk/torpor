...
- docs UI, test utilities, node/cloudflare adapters with auto-detection, and
  ETag/Cache-Control support via `notModified`.
...
5. **Env/secrets API** — there is a `globalThis.adapter.env` convention
   (`src/run/devPlugin.ts:98`), but no unified typed accessor or documented
   dev/prod parity story.

### Medium

6. **Route-scoped middleware** — already a TODO in `src/server/Server.ts:36`;
   right now it is global `use()` or per-folder hooks only.
7. **WebSockets (server-side)** — SSE exists client-side, but no WS endpoint
   support (adapter-node could do this).
8. **SEO helpers** — no sitemap.xml / robots.txt generation, no meta/canonical
   conveniences beyond `@head` merging.
9. **View transitions + scroll restoration** — neither `startViewTransition`
   nor any scroll handling exists in `src/nav/navigate.ts` or
   `src/site/clientEntry.ts`.
10. **i18n / locale routing** — nothing.
11. **Base path config** — `Site` has no `basePath` option (root is always
    `process.cwd()`), so mounting under a subpath is not supported.
12. **Flash messages** — DIY via cookies today; a small helper after
    redirects would round out the actions story.

### Lower / nice-to-have

13. Image optimization / asset pipeline beyond Vite defaults
14. Pagination helpers
15. Rate-limiting primitives (fits naturally as a middleware/plugin)
16. Cron / queues / background jobs (adapter-dependent)
17. Dev toolbar / route inspector
