...
/**
 * Starts the Torpor language server on the current process, wiring up all
 * handlers to a new connection.
 *
 * The transport (stdio, node-ipc or socket) is detected from the command
 * line arguments, with the `torpor-lsp` bin defaulting to stdio.
 */
